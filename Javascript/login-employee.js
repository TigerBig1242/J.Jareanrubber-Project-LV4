...
function login() {
    passaway = true;
    let txt_username = document.querySelector("#username");
    ...
    if(txt_username.value.length <= 0) {
        ...
        txt_username.style.border = "1px solid red";
    }
        ...
        txt_password.style.border = "1px solid red";
    }
    ...
    if(passaway)    {
        ...
        $.ajax({
            type: 'POST',
            url: '../API/login-employee.php',
            data: {
                username: $(txt_username).val(),
                password: MD5($(txt_password).val())
                ...
                if(response.RespCode === 200) {
                  ...
                  if (response.Role === 'พนักงาน') {
                    Swal.fire({
                      icon: 'success',
                      title: 'Success',
                      text: 'Login success'
                    }).then(() => {
                        window.location.href = '../src/employee-confirm.html';
                    })
                  }
                //   else if (response.Role === 'ตัวแทน') {
                //     Swal.fire({
                //       icon: 'success',
                //       title: 'Success',
                //       text: 'Login success'
