                ...
                for($i = 0; $i < count($idStock); $i++) {
                    for($k = 0; $k < count($queryMaterial); $k++) {
                        for($j = 0; $j < count($idStock); $j++) {
                            if($idStock[$i]['id'] == $queryMaterial[$k]['idMat'] && $idStock[$j]['level'] == $queryMaterial[$k]['level']) {
                                // $Amount += intval($queryMaterial[$k]['amount']) * intval($Count);
                                $query = "UPDATE tb_stockmaterial SET Amount_Material = ? WHERE ST_MaterialID = ? AND LeveL_Material = ?";
                                $stmt = $connect->prepare($query);
                                $object = new stdClass;
                                foreach($idStock as $id) {
                                   foreach($sumAmount as $amount) {
                                    foreach($idStock as $level) {
                                        if($stmt->execute([
                                            $amount['grad'], $id['id'], $level['level']
                                        ])) {
