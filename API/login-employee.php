<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        $query = "SELECT * FROM tb_employee WHERE username = ?";
            ...
            if($num > 0) {
                while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    if($encPassword == $password) {
                        ...
                        $query = "UPDATE tb_employee SET token = ? WHERE id_emp = ?";
                        $stmt = $connect->prepare($query);
                        if($stmt->execute([
                            $genToken, $emp_id
                        ])) {
                            ...
                            $object->Log = 4;
                             ...
                             if ($row['status'] == 'พนักงาน') {
                                $object->RespCode = 200;
                                $object->RespMessage = 'Good';
                                $object->Role = 'พนักงาน';
                            }
                            // else if ($row['status'] == 'ตัวแทน') {
                            //     $object->RespCode = 200;
                            //     $object->RespMessage = 'Good';
                            //     $object->Role = 'ตัวแทน';
                            // }
